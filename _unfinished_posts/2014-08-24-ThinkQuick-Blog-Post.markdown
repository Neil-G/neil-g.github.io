...
<p>ThinkQuick! gives you 5 simple arithmetic problems to solve at any given time. Each problem has three numbers and an operation.  If the operation is "ADD", then add the three numbers together, enter the answer in the top-right input box, and press "submit" button.  If the operation is "MULTIPLY," then multiply the numbers instead. If your submitted answer is correct -AND- you are the first to answer correctly, your username (or unknown if you are not logged in) gets added to the Winners column, and the solved problem gets reset.</p>
...
<h2 class="section-heading">SignUp, LogIn, and No Refreshing</h2>
